...
terraform {
  backend "s3" {
    ...
    encrypt = true
    region  = "eu-west-2"
    key     = "services/<%= awsStream %>-<%= projectName %>-header-frontend/state"
  }
}
resource "aws_s3_bucket" "frontend-bucket-production" {
  bucket = "lbh-<%= awsStream %>-<%= projectName %>-frontend-production.hackney.gov.uk"
  acl    = "private"
  versioning {
    enabled = true
  }
...
module "cloudfront-production" {
  ...
  origin_id = "<%= orgName %>-<%= awsStream %>-<%= projectName %>-frontend"
...
resource "aws_ssm_parameter" "cdn" {
  name  = "/<%= awsStream %>/production/<%= projectName %>-app-url"
  type  = "String"
