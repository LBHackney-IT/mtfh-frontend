...
terraform {
  backend "s3" {
    ...
    encrypt = true
    region  = "eu-west-2"
    key     = "services/<%= awsStream %>-<%= projectName %>-frontend/state"
  }
}
resource "aws_s3_bucket" "frontend-bucket-staging" {
  bucket = "lbh-<%= awsStream %>-<%= projectName %>-frontend-staging.hackney.gov.uk"
  acl    = "private"
  versioning {
    enabled = true
  }
...
resource "aws_ssm_parameter" "cdn" {
  name  = "/<%= awsStream %>/staging/<%= projectName %>-app-url"
  type  = "String"
